...
public class GameBoardGenerator {
    ...
    /**
     * Given a x(column) and y(row) find the backend square that corresponds
     *
     * @param x the x
     * @param y the y
     * @return the square
     */
    public Square coordToSquare(int x, int y) {
        int idx = -1;
        ...
        return gameView.getController().getGameManager().getBoard().getSquares()[idx];
    }
     ...
     * @return the coord
     */
    public int[] indexToCoord(int idx) {
        int x = -1;
        int y = -1;
        ...
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 11; j++) {
                ...
                if ((i == 0 && j == 0) || (i == 0 && j == 10) || (i == 10 && j == 0) || (i == 10 && j == 10)) {
                    Square square = coordToSquare(j, i);
